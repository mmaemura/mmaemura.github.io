...
How does the average yearly temperature vary between two countries?
...
def compare_two_countries(countries, year_begin, year_end, month, **kwargs):
    ...
    title = f"{countries[0]} vs. {countries[1]}, Years {year_begin} - {year_end}"
    
    fig = px.line(df, x = "Year", y = "Temperature",
                  facet_col = "Country",
                  facet_col_spacing = .09,
                  title = title,
                  **kwargs)
...
```python
conn = sqlite3.connect("temps.db")
compare_two_countries(("United States", "Canada"), year_begin,
 year_end, month, template = 'plotly_dark')
conn.close()
```
include image
